    ...
    >
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">{t("admin.rolesModal.title")}</h2>
            <p className="text-sm text-gray-600 mt-1">{t("admin.rolesModal.tenant")} {tenantName}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-4 overflow-y-auto max-h-[calc(80vh-140px)]">
          {loading ? (
            <div className="text-center py-8">
              ...
              <p className="mt-2 text-gray-600">{t("admin.rolesModal.loading")}</p>
            </div>
          ) : roles.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              {t("admin.rolesModal.noRoles")}
            </div>
          ) : (
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-400 mt-1">
                        {t("admin.rolesModal.createdAt")} {new Date(role.created_at).toLocaleString("zh-CN")}
                      </div>
                    </div>

                    {role.user_count !== undefined && (
                      <div className="text-sm text-gray-600">
                        {t("admin.rolesModal.usersCount", { count: role.user_count })}
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div className="px-6 py-4 border-t border-gray-200 flex justify-between items-center">
          <div className="text-sm text-gray-600">
            {t("admin.rolesModal.total", { count: roles.length })}
          </div>
          <button
            onClick={onClose}
            className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors"
          >
            {t("admin.rolesModal.close")}
          </button>
        </div>
      </div>
    </div>
